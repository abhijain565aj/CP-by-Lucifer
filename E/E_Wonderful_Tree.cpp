...
void solve()
{
    ...
    fo(i, n)
    {
        pr[i].F = dist[i], pr[i].S = i;
    }
    sortall(pr);
    debug(pr);
    vl minVal(n, 0); // minumum distance of a leaf from node i
    re(i, n)
    {
        if (childs[pr[i].S].size() == 0)
        {
            minVal[pr[i].S] = 0;
        }
        else
        {
            minVal[pr[i].S] = n + 1;
            for (auto x : childs[pr[i].S])
            {
                minVal[pr[i].S] = min(minVal[pr[i].S], minVal[x] + 1);
            }
        }
    }
    debug(minVal);
