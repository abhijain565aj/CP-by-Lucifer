...
*/
ll ans(ll index, ll current, vi &a, vvi &dp)
{
    ...
    ll answer = 1 + ans(index + 1, current + 1, a, dp);
    if (a[index] <= current)
    {
        answer = min(answer, ans(index + 1, current - a[index], a, dp));
    }
...
void solve()
{
    ...
    vvi dp(a.size() + 1, vi(n + 1, -1));
    cout << ans(0, 0, a, dp) << endl;
}
/*
