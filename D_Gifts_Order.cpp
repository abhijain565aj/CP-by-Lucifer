...
void solve()
{
    ...
    fo(i, n) left[i] = a[i] + i, right[i] = a[i] - i;
    debug2(left, right);
    multiset<int> l, r;
    int max1 = left[0];
    vi lmax(n, 0), rmax(n, 0);
    fo(i, n)
    {
        max1 = max(max1, left[i]);
        lmax[i] = max1;
        l.insert(max1 - left[i]);
    }
    max1 = right.back();
    re(i, n)
    {
        ...
        rmax[i] = max1;
        r.insert(max1 - right[i]);
    }
    debug2(l, r);
    cout << max(*l.rbegin(), *r.rbegin()) << endl;
    while (q--)
    {
        int p, x;
        cin >> p >> x;
        p--;
        int oleft = left[p], oright = right[p];
        a[p] = x;
        left[p] = a[p] + p;
        right[p] = a[p] - p;
        cout << -1 << endl;
