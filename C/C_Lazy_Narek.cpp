...
void solve()
{
    ...
    read(a, n);
    string narek = "narek";
    vector<vector<pii>> b(n, vector<pii>(5));
    fo(i, n)
    {
        fo(j, 5)
        {
            ...
            fo(k, m)
            {
                if (a[i][k] != narek[c] && narek.find(a[i][k]) != string::npos)
                    score--;
                else if (a[i][k] == narek[c])
                {
                    c = (c + 1) % 5;
                    score--;
                    if (c == 0)
    ...
    debug(b);
    vvi dp(n, vi(5, -1));
    auto fn = [&](auto &fn, int i, int j)
    {
        if (i == n)
            return 0;
        if (dp[i][j] != -1)
            return dp[i][j];
        dp[i][j] = max(b[i][j].F + fn(fn, i + 1, b[i][j].S), fn(fn, i + 1, j));
        return dp[i][j];
    };
    int ans = 0;
    fo(i, n)
    {
        ans = max(ans, fn(fn, i, 0));
    }
    debug(dp);
    cout << ans << endl;
}
