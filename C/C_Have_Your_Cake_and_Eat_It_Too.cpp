...
void solve()
{
    ...
    //     atleast += a[0][i];
    // }
    ll atleast = accumulate(all(a[0]), 0LL);
    atleast = (atleast + 2) / 3;
    debug(atleast);

    // checking all 6 comb of a,b,c
    vvl o = {{0, 1, 2}, {0, 2, 1}, {1, 0, 2}, {1, 2, 0}, {2, 0, 1}, {2, 1, 0}};
    fo(i, 6)
    {
        vl order = o[i];
        v(pll) ans(3);
        ...
        fo(i, n)
        {
            sum += a[order[current]][i];
            if (sum >= atleast)
            {
                ans[order[current]] = {laston, i + 1};
                laston = i + 2;
                current++;
                sum = 0;
            }
            ...
            {
                ans[order[2]].S = n;
                cout << ans[0].F << " " << ans[0].S << " " << ans[1].F << " " << ans[1].S << " " << ans[2].F << " " << ans[2].S << "\n";
                return;
            }
        }
    }
