...
void solve()
{
    ...
    re(i, n)
    {
        if (a[i].S == 1)
        {
            if (mid >= i)
            {
                best = a[mid + 1].F + a[i].F + k;
            }
            else
            {
                best = a[i].F + a[mid].F + k;
            }
            break;
        }
    }
    ...
    while (before.size() > 0 && k > 0)
    {
        if (a[mid].S == 1)
        {
            ...
            if (mid + in < n - 1)
            {
                a[mid].F = min(a[mid].F + k / in, a[mid + in].F);
                k = max(k - in * (a[mid + in].F - a[mid].F), 0LL);
                a[mid].S = a[mid + in].S;
                in++;
                if (a[mid].S == 1)
                    before.pb(a[mid].F);
            ...
            {
                a[mid].F = a[mid].F + k / in;
                break;
            }
        }
        else
        {
            k -= a[mid].F - before.back();
            a[mid].S = 1;
        }
    }
    ans += a[mid].F;
