...
void solve()
{
    ...
    // i need to find the substring which is closes to the best
    // kmp array of best
    vi kmp(best.size(), 0);
    int j = 0;
    fo1(i, 1, best.size())
    ...
    vi dp(n); // stores the longest prefix of best ending at i
    j = 0;
    fo(i, n)
    {
        if (s[i] == best[j])
    ...
    debug(size);
    debug2(pos, max_len);
    cout << pos - dp[pos] + 2 << " " << pos - dp[pos] + size + 1 << " " << 1 << " " << n << endl;
}
/*
