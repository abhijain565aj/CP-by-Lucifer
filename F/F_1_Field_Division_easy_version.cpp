...
void solve()
{
    ...
    fo(i, k)
    {
        mp[get<0>(a[i])] = max(mp[get<0>(a[i])], get<1>(a[i]));
    }
    debug(mp);
    ll ans = 0;
    ll ly = 1;
    ...
    fo(i, k)
    {
        ans1[get<2>(a[i])] = (mp[get<0>(a[i])] == get<1>(a[i])) ? 1 : 0;
    }
    print_space(ans1, k);
}
/*
