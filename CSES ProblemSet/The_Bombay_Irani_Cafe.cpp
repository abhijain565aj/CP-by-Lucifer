...
void solve()
{
    ...
    ll ans = 0;
    fo(i,n){
        ll left = lower_bound(all(sorted),a[i]-d)-sorted.begin();
        ll self = lower_bound(all(sorted),a[i])-sorted.begin();
        ll right = upper_bound(all(sorted),a[i]+d)-sorted.begin()-1;
        debug3(left,self,right);
        ll val = st.sum(left,right);
        debug(val);
        st.update(self,val+1);
        ans = max(ans,val+1);
    }
    cout<<ans<<endl;
}
