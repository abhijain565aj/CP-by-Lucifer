...
void solve()
{
    ...
    read(a, n);
    vl dp(n, 0);
    vl tail(n + 1, 1e18); // tail[i] is the minimum value a subsequence of length i can end with
    tail[0] = 0;
    fo(i, n)
    {
        auto it = lower_bound(tail.begin(), tail.end(), a[i]) - tail.begin();
        dp[i] = it;
        tail[it] = min(tail[it], a[i]);
    }
