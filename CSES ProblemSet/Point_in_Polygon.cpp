...
void solve()
{
    ...
    auto intersect = [&](int i, pll a) -> ll
    {
        auto p1 = polygon[i % n];
        auto p2 = polygon[(i + 1) % n];
        auto v = (a.F - p1.F) * (p2.S - p1.S) - (a.S - p1.S) * (p2.F - p1.F);
        auto v1 = p1.S - p2.S;
        debug2(v, v1);
        if (v == 0 && (a.F - p1.F) * (a.F - p2.F) <= 0 && (a.S - p1.S) * (a.S - p2.S) <= 0)
            return 0;
        else if (v == 0 || v * v1 < 0 || (a.S - p1.S) * (a.S - p2.S) > 0)
            return -1;
        else
            return 1;
    };
    fo(i, m)
    {
        ...
        fo(j, n)
        {
            if (intersect(j, {a, b}) == 0)
            {
                cout << "BOUNDARY\n";
                cnt = -1;
                break;
            }
            if (intersect(j, {a, b}) == 1)
            {
                cnt++;
                debug2(j, cnt);
            }
