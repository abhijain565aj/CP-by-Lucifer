...
void solve()
{
    ...
    fo(i, 8)
    {
        fo(j, 8)
        {
            board[i][j] = (-1) * (int)(s[i][j] == '*');
        }
    }
    debug(board);
    int ans = 0;
    vb row(8, 0), col(8, 0), d1(15, 0), d2(15, 0);
    auto mark = [&](int i, int j) -> int
    {
        row[i] = true;
        col[j] = true;
        d1[i + j] = true;
    ...
    {
        return !row[i] && !col[j] && !d1[i + j] && !d2[i - j + 7] && board[i][j] != -1;
    };

    auto unmark = [&](int i, int j) -> int
    {
        row[i] = false;
        col[j] = false;
        d1[i + j] = false;
    ...
    function<int(int)> queen = [&](int i) -> int
    {
        debug(i);
        fo(j, 8)
        {
