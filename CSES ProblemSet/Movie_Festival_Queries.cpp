...
struct SegmentTree {
  ...
  SegmentTree(
      int n,
      T default_value = 0,
      function<T(T, T)> merge = [](T l, T r) { return min(l + r); }) {
    this->n = n;
...
signed main() {
  fastio;
  // I come on L then I watch from l upto r
  // n movies and q queries
  // total 4n coordinates (bound by 1e6)

  // for each l to r in that segment I store the best answer for max moviews from i to r for each i (along with the first ending movie starting after their); along with a movie starting movie at the end
}
