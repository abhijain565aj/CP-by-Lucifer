...
//     multiset<int>::iterator itr = total.begin();
//     fo(i,lsize) left.insert(*(itr++));
//     median = *(itr++);
//     fo(i,rsize) right.insert(*(itr++));
//     ans.pb(median);
//     fo1(i,k,n){
//         if(left.find(a[i-k])!=left.end()) left.erase(left.find(a[i-k]));
//         else if(right.find(a[i-k])!=right.end()) right.erase(right.find(a[i-k]));
//         else median = -1;
//         right.insert(a[i]);
//         if(median == -1){
//             median = *right.begin();
...
int main()
{
    ...
    vi ans;
    ordered_set st;
    fo(i,k-1) st.insert({a[i],i});
    fo1(i,k-1,n){
        st.insert({a[i],i});
        ans.pb(st.find_by_order(mid)->first);
