...
struct LazySegmentTree
{
    ...
    void push(ll v)
    {
        tree[v * 2] = operation(tree[v * 2], lazy[v]);
        lazy[v * 2] = operation(lazy[v * 2], lazy[v]);
        tree[v * 2 + 1] = operation(tree[v * 2 + 1], lazy[v]);
        lazy[v * 2 + 1] = operation(lazy[v * 2 + 1], lazy[v]);
        lazy[v] = 0;
    }
    ...
    void updatep(ll v, ll tl, ll tr, ll l, ll r, T val)
    {
        if (l > r) return;
        if (l == tl && r == tr) {
            tree[v] = operation(tree[v], val);
            lazy[v] = operation(lazy[v], val);
        } else {
            ll tm = (tl + tr) / 2;
            push(v);
