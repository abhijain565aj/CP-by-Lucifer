...
void solve()
{
    ...
    debug(a);
    v(vvl) dp(n, vvl(m, vl(m)));
    fo(i, m)
    {
        dp[0][0][i] = a[0][i] + t * i;
        fo1(j, 1, m)
        {
            dp[0][j][i] = a[0][(j + i) % m] + dp[0][j - 1][i];
        }
    }
    vvl dp1(n, vl(m));
    fo(j, m)
    {
        dp1[0][j] = *min_element(all(dp[0][j]));
    }
    fo1(i, 1, n)
    {
        fo(j, m)
        {
            dp[i][0][j] = *min_element(all(dp[i - 1][0])) + a[i][j] + j * t;
        }
    }
    debug(dp);
    debug(dp1);
    fo1(i, 1, n)
    {
        fo1(j, 1, m)
        {
            fo(k, m)
            {
                ...
                dp[i][j][k] = min(dp[i][j][k], dp1[i - 1][j] + a[i][(j + k) % m] + t * k);
            }
        }
        fo(j, m)
        {
            dp1[i][j] = *min_element(all(dp[i][j]));
        }
    }
