...
void solve()
{
    ...
    fo(i, n)
        count[i] = adj[i].size();
    int ans = -1e5;
    auto fn = [&](auto &&fn, int i, int pr) -> int
    {
        vi values = {0, 0};
        for (auto j : adj[i])
        {
            if (j != pr)
            {
                values.push_back(fn(fn, j, i));
            }
        }
        sortall(values);
        ans = max(ans, values[values.size() - 1] + values[values.size() - 2] + count[i] - 2);
        return values[values.size() - 1] + count[i] - 2;
    };
    fn(fn, 0, -1);
    cout << ans + 2 << endl;
}
