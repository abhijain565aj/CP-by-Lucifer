...
// return n_C_r modulo MOD
int c(ll n, ll r, vvi &comb)
{
    // use recursion nd store values of n_C_r in moduler form in comb array
    if (r > n)
    {
        return 0;
    }
    ...
    if (comb[n][r] != -1)
        return comb[n][r];
    return comb[n][r] = (c(n - 1, r - 1, comb) + c(n - 1, r, comb)) % MOD;
}
void solve()
{
    ll n;
    cin >> n;
    vvi comb(n + 1, vi(n + 1, -1));
    ll answer = 0;
    fo(i, n + 1)
    {
        ll max = 2 * i + 1;
        ...
        if (max < n + 1)
        {
            fo(j, i + 1)
            {
                answer += (c(max - 1, i - j, comb) * ((c(n - max, j, comb) * (max)) % MOD)) % MOD;
                max--;
                answer %= MOD;
            }
        }
