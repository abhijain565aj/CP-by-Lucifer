...
*/
bool checkPath(ll &g, int &n, int &m, vvl &a)
{
    v(vb) visited(n, vb(m, false));
    queue<pii> q;
    visited[0][0] = true;
    q.push({0, 0});
    int x, y;
    while (!q.empty())
    {
        ...
        q.pop();
        if (x + 1 < n && !visited[x + 1][y] && a[x + 1][y] % g == 0)
        {
            visited[x + 1][y] = true;
            q.push({x + 1, y});
        }
        if (y + 1 < m && !visited[x][y + 1] && a[x][y + 1] % g == 0)
        {
            visited[x][y + 1] = true;
            q.push({x, y + 1});
        }
