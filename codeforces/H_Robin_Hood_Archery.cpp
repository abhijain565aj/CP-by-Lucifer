...
struct SegmentTreeXOR {
  ...
  pii sump(ll v, ll tl, ll tr, ll l, ll r) {
...
void solve() {
  ...
  fo(i, n) {
    ...
    b[i] = mp[a[i]] = {(int)dist(rng), (int)dist(rng)};
  }
  debug(b);
  SegmentTreeXOR st(n);
  st.build(b);
  while (q--) {
    ...
    cin >> l >> r;
    l--, r--;
    YN((r - l + 1) % 2 == 0 && st.sum(l, r) == make_pair(0LL, 0LL));
  }
}
