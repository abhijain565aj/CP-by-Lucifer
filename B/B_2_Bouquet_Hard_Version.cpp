...
void solve()
{
    ...
    sortall(a);
    ll ans = 0;
    ans = min(a[0].S * a[0].F, (m / a[0].F) * a[0].F);
    fo1(i, 1, n)
    {
        ll curr_ans = 0;
        if (a[i].F - a[i - 1].F > 1)
        {
            curr_ans = min(a[i].S * a[i].F, (m / a[i].F) * a[i].F);
        }
        else if (a[i].F - a[i - 1].F == 1)
        {
            curr_ans = a[i - 1].F * a[i - 1].S + a[i].F * a[i].S;
            if (curr_ans > m)
            {
                curr_ans = a[i - 1].F * a[i - 1].S;
                if (curr_ans > m)
                {
                    curr_ans = m / a[i - 1].F * a[i - 1].F;
                }
                ll u1 = curr_ans / a[i - 1].F;
                ll u2 = (m - curr_ans) / a[i].F;
                curr_ans += u2 * a[i].F;
                ll rem = m - curr_ans;
                rem = min(u1, rem);
                rem = min(rem, a[i].S - u2);
                curr_ans += rem;
            }
        }
        debug(curr_ans);
